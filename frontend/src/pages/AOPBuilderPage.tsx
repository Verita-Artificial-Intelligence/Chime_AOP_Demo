...
const MOCK_PROMPTS: MockPrompt[] = [
    // We will populate this using the mockData
    {
        ...
        description: 'Automate the full onboarding process for a new restaurant, from document verification to menu setup and welcome kit dispatch.',
        config: {
            workflow: 'restaurant-onboarding',
            dataSources: ['Doordash Merchant API', 'Internal CRM', 'Payment Gateway Logs'],
            actions: [
    ...
    },
    {
        ...
        description: 'Dynamically adjust driver dispatch during peak hours using real-time data and predictive modeling to improve delivery times.',
        config: {
            workflow: 'driver-dispatch',
            dataSources: ['Dasher Activity Database', 'Customer Order System', 'Google Maps API', 'Weather Service API'],
            actions: [
    ...
    },
    {
        ...
        description: 'Handle common customer refund scenarios automatically by verifying order details and applying refund policies.',
        config: {
            workflow: 'customer-refund',
            dataSources: ['Customer Order System', 'Payment Gateway Logs', 'Internal CRM'],
            actions: [
    ...
    },
    {
        ...
        description: 'Ensure menu changes from restaurants are reflected instantly across all Doordash platforms and notify Dashers of significant updates.',
        config: {
    ...
    },
    {
        ...
        description: 'Check and flag discrepancies in restaurant operating hours reported versus actual availability.',
        config: {
    ...
    },
    {
        ...
        description: 'Adjust Dasher peak pay incentives in real-time based on sudden demand surges or Dasher shortages.',
        config: {
            workflow: 'dynamic-incentive-adjustment', // Invented workflow ID
            dataSources: ['Customer Order System', 'Dasher Activity Database', 'Weather Service API'],
            actions: [
...
export function AOPBuilderPage() {
  ...
  const addMessage = (sender: ChatMessage['sender'], text: string, config?: AgentConfig) => {
    setMessages(prev => [...prev, { id: config ? 'save-button' : Date.now().toString(), sender, text, timestamp: new Date().toISOString(), config }]);
  };
  ...
  const handlePromptSelect = async (prompt: MockPrompt) => {
    ...
    const workflowDetails = mockData.workflows.find(wf => wf.id === prompt.config.workflow);
    addMessage('agent', `Selecting workflow: ${workflowDetails?.name || prompt.config.workflow}...`);
    ...
    const llmDetails = mockData.llmOptions.find(llm => llm.id === prompt.config.llm);
    addMessage('agent', `Selecting LLM: ${llmDetails?.name || prompt.config.llm}...`);
    await new Promise(resolve => setTimeout(resolve, 1000));
    ...
    addMessage('system', `AOP Configuration for "${prompt.title}" is complete!`, finalAgentConfig);
  ...
  const saveAgent = (agentConfig: AgentConfig) => {
    try {
      ...
      const storedAgents = storedAgentsString ? JSON.parse(storedAgentsString) : [];
      localStorage.setItem('aopAgents', JSON.stringify([...storedAgents, agentConfig]));
      ...
      addMessage('system', `Error saving agent "${agentConfig.name}". Please check console.`);
    }
  };
  ...
  return (
    <div className="container mx-auto max-w-3xl p-4 flex flex-col h-[calc(100vh-100px)] bg-brand-card">
      ...
      {!isBuilding && messages.filter(m => m.id === 'save-button').length === 0 && (
       ...
       {/* Start Over button */}
       {!isBuilding && messages.length > 0 && messages[messages.length -1].id !== 'save-button' && (
