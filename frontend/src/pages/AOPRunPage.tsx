...
function getAOPInstanceById(id: string): AOPInstance | undefined {
  ...
  if (storedHistory) {
    try {
      ...
      console.error('Error parsing aopRunHistory from localStorage on AOPRunPage', e);
...
export default function AOPRunPage() {
  ...
  let runnableConfig: RunnableAgentConfig | undefined = location.state as RunnableAgentConfig | undefined;
  ...
  if (!runnableConfig && routeId) {
    ...
    if (!aopInstanceData) {
      console.warn(`AOPInstance with ID "${routeId}" not found. Displaying mock 'Not Found' data.`);
      aopInstanceData = getAOPInstanceById("AOP-NOT-FOUND-MOCK");
    }
  }

  const isValidRunnableConfig = runnableConfig &&
    typeof runnableConfig.workflow === 'string' && runnableConfig.workflow.trim() !== '' &&
    Array.isArray(runnableConfig.dataSources) &&
    Array.isArray(runnableConfig.actions) &&
    typeof runnableConfig.llm === 'string' && runnableConfig.llm.trim() !== '';
    ...
    return (
      <div className="max-w-3xl mx-auto p-4 md:p-6 bg-white shadow-lg rounded-lg">
        <div className="border-b pb-4 mb-4">
            <h1 className="text-2xl font-bold text-gray-800 mb-1">AOP Run Details: {aopInstanceData.name}</h1>
            <p className="text-sm text-gray-500">ID: {aopInstanceData.id} {aopInstanceData.category && `| Category: ${aopInstanceData.category}`}</p>
            <p className="text-sm text-gray-600 mt-1">{aopInstanceData.description || "No description provided."}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 text-sm">
            <div><span className="font-semibold text-gray-700">Status:</span> 
                <span className={`ml-1 px-2 py-0.5 text-xs font-semibold rounded-full ${ 
                    aopInstanceData.status.toLowerCase().includes('success') || aopInstanceData.status.toLowerCase().includes('active') || aopInstanceData.status.toLowerCase().includes('completed') ? 'bg-green-100 text-green-800' :
                    aopInstanceData.status.toLowerCase().includes('fail') || aopInstanceData.status.toLowerCase().includes('error') ? 'bg-red-100 text-red-800' :
                    'bg-yellow-100 text-yellow-800' 
                  }`}>
                  {aopInstanceData.status}
                </span>
            </div>
            <div><span className="font-semibold text-gray-700">Last Run:</span> {new Date(aopInstanceData.lastRun).toLocaleString()}</div>
        </div>

        {aopInstanceData.metrics && Object.keys(aopInstanceData.metrics).length > 0 && (
            <div className="mb-6">
                <h2 className="text-lg font-semibold text-gray-700 mb-2">Metrics</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-2 text-sm bg-gray-50 p-4 rounded-md">
                    {Object.entries(aopInstanceData.metrics).map(([key, value]) => (
                        value !== undefined && <div key={key}><span className="font-medium text-gray-600">{key.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())}:</span> {value}</div>
                    ))}
                </div>
            </div>
        )}

        <h2 className="text-lg font-semibold text-gray-700 mb-3">Execution Log</h2>
        {aopInstanceData.runHistory && aopInstanceData.runHistory.length > 0 ? (
          <ul className="space-y-3 border rounded-md p-4 bg-gray-50 max-h-96 overflow-y-auto">
            {aopInstanceData.runHistory.map((entry, index) => (
              <li key={index} className="text-sm pb-3 border-b border-gray-200 last:border-b-0 last:pb-0">
                <div className="flex justify-between items-center mb-0.5">
                  <span className={`font-semibold ${entry.status.toLowerCase() === 'success' ? 'text-green-600' : entry.status.toLowerCase() === 'failed' ? 'text-red-600' : 'text-yellow-600'}`}>
                    {entry.status}
                  </span>
          ...
          <p className="text-gray-500 text-sm">No execution log entries found for this run.</p>
        )}
  ...
  return (
    <div className="max-w-2xl mx-auto py-16 text-center">
      <div className="text-xl text-red-500 font-semibold mb-4">Agent Configuration or Run Data Not Found.</div>
      <p className="text-gray-600 mb-6">Could not find details for ID: {routeId}. It may have been deleted or the ID is incorrect.</p>
      <button 
        className="px-6 py-2 bg-red-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
